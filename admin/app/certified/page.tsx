...
function DaysLeftBadge({ days }: { days: number }) {
  if (days <= 0)
    return (
      <span
        ...
        style={{ color: "#D93025", background: "#FDECEA" }}
      >
        만료됨
      </span>
    );
  if (days <= 30)
    return (
      <span
        className="inline-flex items-center px-2.5 py-0.5 rounded-lg text-xs font-bold"
        style={{ color: "#E37400", background: "#FEF3E2" }}
      >
        {days}일 남음
      </span>
    );
...
function MaskKey({ apiKey }: { apiKey: string }) {
  const [revealed, setRevealed] = useState(false);
  const [copied, setCopied] = useState(false);

  const display = revealed ? apiKey : `${apiKey.slice(0, 8)}${"•".repeat(24)}`;
...
interface AddModalProps {
  onClose: () => void;
  onCreated: (name: string, affiliation: string) => Promise<string>;
}
...
function AddModal({ onClose, onCreated }: AddModalProps) {
  ...
  const submit = async () => {
    ...
    setLoading(true);
    try {
      const key = await onCreated(name.trim(), affiliation.trim());
      setNewKey(key);
    } finally {
      setLoading(false);
    }
        ...
        {newKey ? (
          <>
            ...
            <p className="text-sm text-gray-500 mb-4">
              이 키는 다시 확인할 수 없으니 지금 바로 복사해서 전달하세요.
            </p>
            <div
...
interface EditModalProps {
  user: CertifiedUser;
  onClose: () => void;
  onSaved: (id: number, name: string, affiliation: string) => Promise<void>;
}
...
function EditModal({ user, onClose, onSaved }: EditModalProps) {
  ...
  const submit = async () => {
    ...
    setLoading(true);
    try {
      await onSaved(user.id, name.trim(), affiliation.trim());
      onClose();
    } finally {
      setLoading(false);
    }
    ...
    >
      <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md mx-4">
        ...
        <div className="flex flex-col gap-3 mb-6">
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-1">소속 / 기관</label>
            <input
              value={affiliation}
              onChange={(e) => setAffiliation(e.target.value)}
              className="w-full px-3 py-2.5 rounded-xl border text-sm outline-none"
...
export default function CertifiedPage() {
  ...
  const handleEdit = async (id: number, name: string, affiliation: string) => {
    await updateCertifiedUser(id, name, affiliation);
    await load();
  };

  const handleDelete = async (user: CertifiedUser) => {
    if (!confirm(`"${user.name}" 인증 사용자를 삭제하시겠습니까?\n발급된 키가 즉시 무효화됩니다.`))
      return;
  ...
  return (
    <div>
      ...
      >
        <strong>유효 기간:</strong> 발급일로부터 365일 · 만료된 키는 자동으로 비활성화됩니다.
        키 분실 시 해당 사용자를 삭제 후 재등록하세요.
      </div>
            ...
            <tbody>
              {users.map((u, i) => (
                ...
                >
                  ...
                  <td className="px-5 py-4 text-gray-500">{u.affiliation || "—"}</td>
                  <td className="px-5 py-4">
                    <MaskKey apiKey={u.apiKey} />
                  </td>
                  <td className="px-5 py-4">
                    <DaysLeftBadge days={u.daysLeft} />
                  </td>
                  <td className="px-5 py-4">
